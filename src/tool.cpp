...
Tool::Tool(const std::string& name, const std::string& description, bool operands_reqd, std::ostream& os ) :
    option_string_{":"},             // empty options with self-generated error suppression.
    name_{name},
    description_{description},
    os_{os},
    operands_reqd_{ operands_reqd }
{}
...
Tool& Tool::addOption(char short_name, const std::string& long_name, const std::string& description, int hasArg, const std::string& defaultArg )
{
    ...
    // need an option_string_ described as follows:
    ...
    option_vector_.push_back( o );                      // this must make a copy (hence push_back), so we can use it outside of the scope of this method.
...
bool Tool::parseArgs(int argc, char* argv[])
{
    ...
    while ((opt = getopt_long(argc, argv, option_string_.c_str(), &option_vector_[0], nullptr)) != -1)
    {
        /**
         ...
         * -1  : end of options.
         * 0   : set the flag variable in the option_vector_ structure.
